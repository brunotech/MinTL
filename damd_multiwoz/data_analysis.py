...
def analysis():
        ...
        for turn in logs:
            if not turn['metadata']:
                ...
                for dom, book_semi in meta.items():
                    ...
                    record = any(value not in ['', []] for slot, value in book.items())
                    if record:
                        turn_dict['metadata'][dom] = {'book': book}
                    record = any(value not in ['', []] for slot, value in semi.items())
                    if record:
            ...
            for dom in dial_domains:
                if not dom_count.get(f'{dom}_{dial_type}'):
                    dom_count[f'{dom}_{dial_type}'] = 1
                else:
                    dom_count[f'{dom}_{dial_type}'] += 1
                if not dom_fnlist.get(f'{dom}_{dial_type}'):
                    dom_fnlist[f'{dom}_{dial_type}'] = [fn]
                else:
                    dom_fnlist[f'{dom}_{dial_type}'].append(fn)
                dom_str += f'{dom}_'
            dom_str = dom_str[:-1]
            if dial_type=='multi':
            ...
            for domain in domains:
                ...
                for info_s in info_ss:
                    all_domain_specific_slots.add(f'{domain}-{info_s}')
                    if info_s not in info_slots[domain]:
                        info_slots[domain]+= [info_s]
                for book_s in book_ss:
                    if f'book_{book_s}' not in info_slots[
                        domain
                    ] and book_s not in ['invalid', 'pre_invalid']:
                        all_domain_specific_slots.add(f'{domain}-{book_s}')
                        info_slots[domain] += [f'book_{book_s}']
                for req_s in req_ss:
    ...
    with open(f'{save_path}req_slots.json', 'w') as sf:
        json.dump(req_slots,sf,indent=2)
    with open(f'{save_path}info_slots.json', 'w') as sf:
        json.dump(info_slots,sf,indent=2)
    with open(f'{save_path}all_domain_specific_info_slots.json', 'w') as sf:
        json.dump(list(all_domain_specific_slots),sf,indent=2)
        print("slot num:", len(list(all_domain_specific_slots)))
    with open(f'{save_path}goal_of_each_dials.json', 'w') as sf:
        json.dump(goal_of_dials, sf, indent=2)
    with open(f'{save_path}compressed_data.json', 'w') as sf:
        json.dump(compressed_raw_data, sf, indent=2)
    with open(f'{save_path}domain_count.json', 'w') as sf:
    ...
    with open(f'{save_path_exp}reference_no.json', 'w') as sf:
        json.dump(ref_nos,sf,indent=2)
    with open(f'{save_path_exp}domain_files.json', 'w') as sf:
